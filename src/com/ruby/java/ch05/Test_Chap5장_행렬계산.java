...
public static void main(String[] args) {
		...
		System.out.println("05행렬계산_행렬 더하기: A[3][4] = B[3][4] + C[3][4]");
		getData(B);
		getData(C);
		getData(E);
		A = addMatrix(B, C);
		...
		showMatrix(C);
		showMatrix(A);
		System.out.println("05행렬계산_행렬 곱하기: D[3][5] = B[3][4] * E[4][5]");
		D = multiplyMatrix(B,E);
		showMatrix(B);
		showMatrix(E);
		showMatrix(D);
		System.out.println("05행렬계산_행렬 전치: F[4][3] = B[3][4]의 전치 행렬");
		F = transposeMatrix(B);
		showMatrix(B);
		showMatrix(F);
